// export const Url = "http://localhost:5000/api/v1";
//export const Url1 = "https://doctor-app-backend-liard.vercel.app/api/v1";
export const Url = "https://doctor-app-backend-eight.vercel.app/api/v1";

export const REGISTER_USER_REQUEST = "REGISTER_USER_REQUEST";
export const REGISTER_USER_SUCCESS = "REGISTER_USER_SUCCESS";
export const REGISTER_USER_FAIL = "REGISTER_USER_FAIL";

export const ENROLMENT_PATIENT_REQUEST = "ENROLMENT_PATIENT_REQUEST";
export const ENROLMENT_PATIENT_SUCCESS = "ENROLMENT_PATIENT_SUCCESS";
export const ENROLMENT_PATIENT_FAIL = "ENROLMENT_PATIENT_FAIL";
export const ENROLMENT_PATIENT_RESET = "ENROLMENT_PATIENT_RESET";

export const GET_ALL_PATIENT_REQUEST = "GET_ALL_PATIENT_REQUEST";
export const GET_ALL_PATIENT_SUCCESS = "GET_ALL_PATIENT_SUCCESS";
export const GET_ALL_PATIENT_FAIL = "GET_ALL_PATIENT_FAIL";

export const UPDATE_PATIENT_REQUEST = "UPDATE_PATIENT_REQUEST";
export const UPDATE_PATIENT_SUCCESS = "UPDATE_PATIENT_SUCCESS";
export const UPDATE_PATIENT_FAIL = "UPDATE_PATIENT_FAIL";
export const UPDATE_PATIENT_RESET = "UPDATE_PATIENT_RESET";

export const GET_PATIENT_DETAILS_REQUEST = "GET_PATIENT_DETAILS_REQUEST";
export const GET_PATIENT_DETAILS_SUCCESS = "GET_PATIENT_DETAILS_SUCCESS";
export const GET_PATIENT_DETAILS_FAIL = "GET_PATIENT_DETAILS_FAIL";

export const GET_ALL_PATIENT_FORMS_REQUEST = "GET_ALL_PATIENT_FORMS_REQUEST";
export const GET_ALL_PATIENT_FORMS_SUCCESS = "GET_ALL_PATIENT_FORMS_SUCCESS";
export const GET_ALL_PATIENT_FORMS_FAIL = "GET_ALL_PATIENT_FORMS_FAIL";

export const CREATE_APPOINTMENT_REQUEST = "CREATE_APPOINTMENT_REQUEST";
export const CREATE_APPOINTMENT_SUCCESS = "CREATE_APPOINTMENT_SUCCESS";
export const CREATE_APPOINTMENT_FAIL = "CREATE_APPOINTMENT_FAIL";
export const CREATE_APPOINTMENT_RESET = "CREATE_APPOINTMENT_RESET";

export const GET_APPOINTMENT_REQUEST = "GET_APPOINTMENT_REQUEST";
export const GET_APPOINTMENT_SUCCESS = "GET_APPOINTMENT_SUCCESS";
export const GET_APPOINTMENT_FAIL = "GET_APPOINTMENT_FAIL";

export const USER_LOGIN_REQUEST = "USER_LOGIN_REQUEST";
export const PATIENT_LOGIN_SUCCESS = "PATIENT_LOGIN_SUCCESS";
export const PAT_LOGIN_SUCCESS = "PATIENT_LOGIN_SUCCESS";
export const DOCTOR_LOGIN_SUCCESS = "DOCTOR_LOGIN_SUCCESS";
export const USER_LOGIN_FAIL = "USER_LOGIN_FAIL";
export const PATIENT_SIGNOUT = "PATIENT_SIGNOUT";
export const DOCTOR_SIGNOUT = "DOCTOR_SIGNOUT";

export const SENDOTP_REQUEST = "SENDOTP_REQUEST";
export const SENDOTP_SUCCESS = "SENDOTP_SUCCESS";
export const SENDOTP_FAIL = "SENDOTP_FAIL";
export const SENDOTP_RESET = "SENDOTP_RESET";

export const GET_PRESCRIPTION_REQUEST = "GET_PRESCRIPTION_REQUEST";
export const GET_PRESCRIPTION_SUCCESS = "GET_PRESCRIPTION_SUCCESS";
export const GET_PRESCRIPTIONT_FAIL = "GET_PRESCRIPTION_FAIL";

export const CREATE_OBSERVATION_REQUEST = "CREATE_OBSERVATION_REQUEST";
export const CREATE_OBSERVATION_SUCCESS = "CREATE_OBSERVATION_SUCCESS";
export const CREATE_OBSERVATION_FAIL = "CREATE_OBSERVATION_FAIL";
export const CREATE_OBSERVATION_RESET = "CREATE_OBSERVATION_RESET";

export const GET_OBSERVATION_REQUEST = "GET_OBSERVATION_REQUEST";
export const GET_OBSERVATION_SUCCESS = "GET_OBSERVATION_SUCCESS";
export const GET_OBSERVATION_FAIL = "GET_OBSERVATION_FAIL";

export const GET_PATIENT_PROFILE_REQUEST = "GET_PATIENT_PROFILE_REQUEST";
export const GET_PATIENT_PROFILE_SUCCESS = "GET_PATIENT_PROFILE_SUCCESS";
export const GET_PATIENT_PROFILE_FAIL = "GET_PATIENT_PROFILE_FAIL";

export const SUBMIT_FORM_REQUEST = "SUBMIT_FORM_REQUEST";
export const SUBMIT_FORM_SUCCESS = "SUBMIT_FORM_SUCCESS";
export const SUBMIT_FORM_FAIL = "SUBMIT_FORM_FAIL";
export const SUBMIT_FORM_RESET = "SUBMIT_FORM_RESET";

export const UPDATE_FORM_REQUEST = "UPDATE_FORM_REQUEST";
export const UPDATE_FORM_SUCCESS = "UPDATE_FORM_SUCCESS";
export const UPDATE_FORM_FAIL = "UPDATE_FORM_FAIL";
export const UPDATE_FORM_RESET = "UPDATE_FORM_RESET";

export const GET_LATEST_PRESCRIPTION_REQUEST =
  "GET_LATEST_PRESCRIPTION_REQUEST";
export const GET_LATEST_PRESCRIPTION_SUCCESS =
  "GET_LATEST_PRESCRIPTION_SUCCESS";
export const GET_LATEST_PRESCRIPTIONT_FAIL = "GET_LATEST_PRESCRIPTION_FAIL";

export const GET_LATEST_DIET_CHART_REQUEST = "GET_LATEST_DIET_CHART_REQUEST";
export const GET_LATEST_DIET_CHART_SUCCESS = "GET_LATEST_DIET_CHART_SUCCESS";
export const GET_LATEST_DIET_CHART_FAIL = "GET_LATEST_DIET_CHART_FAIL";

export const GET_PATIENT_DIET_CHART_REQUEST = "GET_PATIENT_DIET_CHART_REQUEST";
export const GET_PATIENT_DIET_CHART_SUCCESS = "GET_PATIENT_DIET_CHART_SUCCESS";
export const GET_PATIENT_DIET_CHART_FAIL = "GET_PATIENT_DIET_CHART_FAIL";

export const GET_ANS_FORMS_REQUEST = "GET_ANS_FORMS_REQUEST";
export const GET_ANS_FORMS_SUCCESS = "GET_ANS_FORMS_SUCCESS";
export const GET_ANS_FORMS_FAIL = "GET_ANS_FORMS_FAIL";
