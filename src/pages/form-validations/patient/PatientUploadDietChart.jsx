...
const PatientUploadDietChart = () => {
  ...
  // const deitChartLatest = patientdietchart
  //   ? patientdietchart[patientdietchart.length - 1]
  //   : [];

  useEffect(() => {
    const user = "Patient";
    dispatch(getDietChartOfPatient(user));

    // dispatch(getAllDietChart(user));
    // dispatch(getLatesDietChartByDoctor(patId));
    // if (success) {
    //   dispatch({ type: UPLOAD_DIET_CHART_RESET });
    // }
  }, [dispatch]);
            ...
            {loading ? (
              ...
              <>
                ...
                <a
                  ...
                  type="button"
                  className="ml-4 px-6 py-3 bg-emerald-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-emerald-700 hover:shadow-lg focus:bg-emerald-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-emerald-800 active:shadow-lg transition duration-150 ease-in-out"
      ...
      >
        <div className="relative w-full max-w-2xl max-h-full">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            {console.log("patientdietcharts ==>", patientdietcharts)}
            <div className="p-6 text-xl space-y-6">
              {loading ? (
                <LoadingBox></LoadingBox>
              ) : error ? (
                <MessageBox>{error}</MessageBox>
              ) : patientdietcharts?.length > 0 ? (
                patientdietcharts.map((dt) => (
                  <>
                      ...
                      <div className="relative w-full overflow-hidden">
                        ...
                        <div className="bg-white shadow-lg rounded-b-md overflow-hidden transition-all duration-500 max-h-0 peer-checked:max-h-max">
                          <div className="p-4" ref={componentRef2}>
                            {dt?.dietChartId?.status === "De-Active" ? (
                              <p className="text-red-500">
                                The new diet chart will be attached.
                              </p>
                            ) : (
                              <div className="form__Grid--Cols-6">
                                <div className="form__Cols--Span-6">
                          ...
                          {dt?.dietChartId?.status !== "De-Active" && (
                            <a
                              ...
                              type="button"
                              className="px-6 py-2.5 bg-emerald-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-emerald-700 hover:shadow-lg focus:bg-emerald-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-emerald-800 active:shadow-lg transition duration-150 ease-in-out ml-4"
                              style={{ marginBottom: "16px", width: "200px" }}
                            >
                              Download Diet Charts!
                            </a>
